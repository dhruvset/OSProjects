...
// 
	...
	for(j=i;j<=i;j++)
	{
	  if(strcmp(ff[min].filename,ff[j].filename)<0)
	  {
	    strcpy(temp,ff[min].filename);
	    strcpy(ff[j].filename,ff[min].filename);
	    strcpy(ff[min].filename,temp);
	    min=j;
	  }
	}
      }
...
void send_404(char cmdtype[4],char path[SIZE],int cfd,char clientip[100], time_t arrtime)
{
  ...
  fprintf(cfp,"<html><body><title>NOT FOUND<title><H1>File not found: %s</H1></body></html>",path);
